...
StorageMgr::StorageMgr() { InitializeCriticalSection(&m_csOperation); }
...
UINT32 StorageMgr::UploadFile(ServerAddress *pStorageAddr, const TCHAR *szGroupName, UINT32 nStorePathIndex, const BYTE *pbyFileBuff, UINT32 nFileSize,
    ...
    *p++ = (BYTE)nStorePathIndex;
    ...
    if (pbyFileExtName != NULL) {
        size_t nFileExtLen = strlen((const char *)pbyFileExtName);
    ...
    if ((nRet = tcpsenddata_nb(pStorageServer->sock, byOutBuff, p - byOutBuff, DEFAULT_NETWORK_TIMEOUT)) != 0) {
        ...
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::UploadFile tcpsenddata_nb Header Failed, ")
                     _T("Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if ((nRet = tcpsenddata_nb(pStorageServer->sock, (char *)pbyFileBuff, nFileSize, DEFAULT_NETWORK_TIMEOUT)) != 0) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::UploadFile tcpsenddata_nb Body Failed, ")
                     _T("Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if ((nRet = tcprecvdata_nb(pStorageServer->sock, &resp, sizeof(resp), DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::UploadFile tcprecvdata_nb ID Header ")
    ...
    if (resp.byStatus != 0) {
        ...
        if (resp.byStatus == 28) {
            WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                         _T("StorageMgr::UploadFile FastDFS Cluster Don't ")
                         _T("Have Enough Space"),
                         pbyGroupName, pbyRemoteFileName);
    ...
    UINT64 nInBytes = buff2long64((const char *)(resp.byPkgLen));
    if (nInBytes < 0) {
        closesocket(pStorageServer->sock);
        pStorageServer->sock = INVALID_SOCKET;
        LeaveCriticalSection(&pStorageServer->csRecv);
    ...
    if ((nRet = tcprecvdata_nb(pStorageServer->sock, pbyInBuff, nInBytes, DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::UploadFile tcprecvdata_nb ID Body ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        delete[] pbyInBuff;
        return enumNetworkError_FDFS;
    ...
    memcpy(pbyRemoteFileName, pbyInBuff + FDFS_GROUP_NAME_MAX_LEN, nInBytes - FDFS_GROUP_NAME_MAX_LEN);
...
UINT32 StorageMgr::UploadSlaveFile(const ServerAddress *pStorageAddr, const BYTE *pbyFileBuff, UINT64 nFileSize, const BYTE *pbyMasterFileName, const BYTE *pbyPrefixName,
                                   const BYTE *pbyFileExtName, BYTE *pbyGroupName, BYTE *pbyRemoteFileName) {
    UINT32 nRet;
    size_t nMasterFileNameLen;
    size_t nPrefixNameLen;
    BYTE byOutBuff[512];
    BYTE *p;
    ConnectionInfo *pStorageServer = NULL;
    pbyGroupName[0] = '\0';
    ...
    if (pStorageServer == NULL) {
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::UploadFile Connect Storage Failed Server ")
                     _T("IP:%s, Port:%d"),
                     pStorageAddr->szIP, pStorageAddr->nPort);
    ...
    if ((nRet = tcpsenddata_nb(pStorageServer->sock, byOutBuff, p - byOutBuff, DEFAULT_NETWORK_TIMEOUT)) != 0) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::UploadSlaveFile tcpsenddata_nb Header ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if ((nRet = tcpsenddata_nb(pStorageServer->sock, (char *)pbyFileBuff, nFileSize, DEFAULT_NETWORK_TIMEOUT)) != 0) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::UploadSlaveFile tcpsenddata_nb Body ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if ((nRet = tcprecvdata_nb(pStorageServer->sock, &resp, sizeof(resp), DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::UploadSlaveFile tcprecvdata_nb ID Header ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if (resp.byStatus != 0) {
        ...
        if (resp.byStatus == 28) {
            WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                         _T("StorageMgr::UploadSlaveFile FastDFS Cluster ")
                         _T("Don't Have Enough Space"),
                         pbyGroupName, pbyRemoteFileName);
            return enumNoEnoughSpace_FDFS;
        } else {
    ...
    if ((nRet = tcprecvdata_nb(pStorageServer->sock, pbyInBuff, nInBytes, DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::UploadSlaveFile tcprecvdata_nb ID Body ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        delete[] pbyInBuff;
        return enumNetworkError_FDFS;
...
UINT32 StorageMgr::DownloadFile(const ServerAddress *pStorageAddr, const BYTE *pbyGroupName, const BYTE *pbyRemoteFileName, BYTE *pbyFileBuff, UINT64 *nFileSize) {
    UINT32 nRet;
    ...
    if ((nRet = tcpsenddata_nb(pStorageServer->sock, byOutBuff, p - byOutBuff, DEFAULT_NETWORK_TIMEOUT)) != 0) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::DownloadFile tcpsenddata_nb Header ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if ((nRet = tcprecvdata_nb(pStorageServer->sock, &resp, sizeof(resp), DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::DownloadFile tcprecvdata_nb Header ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if ((nRet = tcprecvdata_nb(pStorageServer->sock, pbyFileBuff, nInBytes, DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::DownloadFile tcprecvdata_nb Body Failed, ")
                     _T("Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
...
UINT32 StorageMgr::ConnectToStorage(ConnectionInfo *pStorageServer) {
    ...
    if (ioctlsocket(pStorageServer->sock, FIONBIO, (u_long *)&nFlag) == SOCKET_ERROR) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::ConnectToStorage ioctlsocket Failed, ")
                     _T("Error Code:%d"),
                     WSAGetLastError());
        return enumFailure_FDFS;
    }
...
ConnectionInfo *StorageMgr::GetConnection(const ServerAddress *pStorageServer) {
    ...
    for (int i = 0; i < nDeqSize; i++) {
        ...
        if (pConnectionInfo->port == pStorageServer->nPort && 0 == strcmp(pConnectionInfo->ip_addr, (const char *)pStorageServer->szIP)) {
    ...
    if (nRet == enumSuccess_FDFS) {
        ...
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::GetConnection ConnectToStorage Failed ")
                     _T("Server IP:%s, Port:%d"),
                     pStorageServer->szIP, pStorageServer->nPort);
...
UINT32 StorageMgr::DeleteFile(const ServerAddress *pStorageAddr, const BYTE *pbyGroupName, const BYTE *pbyRemoteFileName) {
    ...
    if (pStorageServer == NULL) {
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::DeleteFile Connect Storage Failed Server ")
                     _T("IP:%s, Port:%d"),
                     pStorageAddr->szIP, pStorageAddr->nPort);
        return enumFailure_FDFS;
    }
    ...
    if ((nRet = tcpsenddata_nb(pStorageServer->sock, byOutBuff, nOutBytes, DEFAULT_NETWORK_TIMEOUT)) != 0) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::DeleteFile tcpsenddata_nb Header Failed, ")
                     _T("Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if ((nRet = tcprecvdata_nb(pStorageServer->sock, &resp, sizeof(resp), DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pStorageServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("StorageMgr::DeleteFile tcprecvdata_nb Header Failed, ")
                     _T("Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
