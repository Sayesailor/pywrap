...
UINT32 buff2int(const char *buff) {
    return (((unsigned char)(*buff)) << 24) | (((unsigned char)(*(buff + 1))) << 16) | (((unsigned char)(*(buff + 2))) << 8) | ((unsigned char)(*(buff + 3)));
}

UINT64 __cdecl buff2long64(const char *buff) {
    unsigned char *p;
    p = (unsigned char *)buff;
    return (((UINT64)(*p)) << 56) | (((UINT64)(*(p + 1))) << 48) | (((UINT64)(*(p + 2))) << 40) | (((UINT64)(*(p + 3))) << 32) | (((UINT64)(*(p + 4))) << 24) |
...
void __cdecl WriteLogInfo(const TCHAR *pszFileName, int nLogLevel, const TCHAR *pszLogInfo, ...) {
    ...
    if (pszFileName == NULL || pszFileName[0] == _T('\0') || pszLogInfo == NULL || pszLogInfo[0] == _T('\0') || _tcslen(pszLogInfo) >= LOG_INFO_MAX_SIZE) return;
