...
TrackerMgr::~TrackerMgr() { DeleteCriticalSection(&m_csOperation); }
...
UINT32 TrackerMgr::Initialize(ServerAddress *pAddr, UINT32 nArrCount) {
    ...
    for (i = 0; i < nArrCount; i++) {
        ...
        strcpy_s(pCurrentConn->ip_addr, sizeof(pCurrentConn->ip_addr), (char *)pCurrentAddr->szIP);
        ...
        if (nRet != enumSuccess_FDFS) {
            WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                         _T("TrackerMgr::Initialize Connect Failed IP:%s, ")
                         _T("Port:%d, Return Code:%d"),
                         pCurrentConn->ip_addr, pCurrentConn->port, nRet);
...
ConnectionInfo *TrackerMgr::GetConnection() {
    ...
    UINT32 nDeqSize = (UINT32)m_deqConnectionInfo.size();
    if (nDeqSize == 0) return NULL;
    if (m_nLoop < nDeqSize)
    ...
    if (pRet->sock == INVALID_SOCKET) ConnectToTracker(pRet);
    return pRet;
}
...
ConnectionInfo *TrackerMgr::GetConnectionByAddr(ServerAddress *pTrackerAddr) {
    UINT32 nDeqSize = (UINT32)m_deqConnectionInfo.size();
    UINT32 nRet;
    for (UINT32 i = 0; i < nDeqSize; i++) {
        ConnectionInfo *pConnectionInfo = m_deqConnectionInfo[i];
        if (pConnectionInfo->port == pTrackerAddr->nPort && 0 == strcmp(pConnectionInfo->ip_addr, (const char *)pTrackerAddr->szIP)) {
    ...
    if (nRet == enumSuccess_FDFS) {
        ...
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::GetConnectionByAddr ConnectToTracker ")
                     _T("Failed Server IP:%s, Port:%d"),
                     pTrackerAddr->szIP, pTrackerAddr->nPort);
...
UINT32 TrackerMgr::QueryStorageStore(ConnectionInfo *pTrackerServer, ServerAddress *pStorageServer, TCHAR *szGroupName, UINT32 *nStorePathIndex) {
    UINT32 nRet;
    UINT64 nInBytes;
    ...
    if ((nRet = tcpsenddata_nb(pTrackerServer->sock, &header, sizeof(header), DEFAULT_NETWORK_TIMEOUT)) != 0) {
        ...
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::QueryStorageStore tcpsenddata_nb Header ")
                     _T("Failed, Error Code:%d"),
                     nRet);
    ...
    if ((nRet = tcprecvdata_nb(pTrackerServer->sock, &resp, sizeof(resp), DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::QueryStorageStore tcprecvdata_nb Header ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if (resp.byStatus != 0) {
        ...
        if (resp.byStatus == 28) return enumNoEnoughSpace_FDFS;
        return enumFailure_FDFS;
    }
    ...
    if ((nRet = tcprecvdata_nb(pTrackerServer->sock, pbyBuff, nInBytes, DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::QueryStorageStore tcprecvdata_nb Body ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        delete[] pbyBuff;
        return enumNetworkError_FDFS;
    ...
    *(szGroupName + FDFS_GROUP_NAME_MAX_LEN) = '\0';
    ...
    pStorageServer->nPort = static_cast<UINT32>(buff2long64((const char *)(pbyBuff + FDFS_GROUP_NAME_MAX_LEN + IP_ADDRESS_SIZE - 1)));
...
UINT32 TrackerMgr::QueryUpdateStorageStore(ConnectionInfo *pTrackerServer, const BYTE *pbyMasterGroupName, const BYTE *pbyMasterFileName, ServerAddress *pStorageServer,
    ...
    size_t nMasterFileNameLen = strlen((const char *)pbyMasterFileName);
    ...
    if ((nRet = tcpsenddata_nb(pTrackerServer->sock, &byOutBuff, sizeof(TrackerHeader) + FDFS_GROUP_NAME_MAX_LEN + nMasterFileNameLen, DEFAULT_NETWORK_TIMEOUT)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::QueryUpdateStorageStore tcpsenddata_nb ")
                     _T("Header Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if ((nRet = tcprecvdata_nb(pTrackerServer->sock, &resp, sizeof(resp), DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::QueryUpdateStorageStore tcprecvdata_nb ")
                     _T("Header Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
...
UINT32 TrackerMgr::QueryStorageFetch(ConnectionInfo *pTrackerServer, const BYTE *szGroupName, const BYTE *szRemoteFileName, ServerAddress *pStorageServer) {
    size_t nRemoteFileNameLen;
    size_t nGroupNameLen;
    size_t nOutBytes = 0;
    size_t nInBytes = 0;
    UINT32 nRet;
    BYTE *p = NULL;
    ...
    if ((nRet = tcpsenddata_nb(pTrackerServer->sock, byOutBuff, nOutBytes, DEFAULT_NETWORK_TIMEOUT)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::QueryStorageFetch tcpsenddata_nb Header ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if ((nRet = tcprecvdata_nb(pTrackerServer->sock, &resp, sizeof(resp), DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::QueryStorageFetch tcprecvdata_nb Header ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if ((nRet = tcprecvdata_nb(pTrackerServer->sock, pbyBuff, nInBytes, DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::QueryStorageFetch tcprecvdata_nb Body ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        delete[] pbyBuff;
        return enumNetworkError_FDFS;
...
UINT32 TrackerMgr::ConnectToTracker(ConnectionInfo *pTrackerServer) {
    ...
    inet_pton(AF_INET, cIP, &(s_in.sin_addr));
    ...
    if (connect(pTrackerServer->sock, (struct sockaddr *)&s_in, sizeof(struct sockaddr)) == SOCKET_ERROR) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::ConnectToTracker Can't Connect Tracker ")
                     _T("Server IP:%s, Port:%d"),
...
UINT32 TrackerMgr::GetGroupStat(ConnectionInfo *pTrackerServer, FDFSGroupStat *pStat, UINT32 nLen, UINT32 *pnStatCount) {
    UINT32 nRet;
    UINT64 nInBytes;
    ...
    if ((nRet = tcpsenddata_nb(pTrackerServer->sock, &header, sizeof(header), DEFAULT_NETWORK_TIMEOUT)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::GetGroupStat tcpsenddata_nb Header ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if ((nRet = tcprecvdata_nb(pTrackerServer->sock, &resp, sizeof(resp), DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::GetGroupStat tcprecvdata_nb Header ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    *pnStatCount = static_cast<UINT32>(nInBytes / sizeof(GroupStat));
    ...
    if ((nRet = tcprecvdata_nb(pTrackerServer->sock, rawStat, nInBytes, DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::GetGroupStat tcprecvdata_nb Body Failed, ")
                     _T("Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    for (UINT32 i = 0; i < *pnStatCount; i++) {
...
UINT32 TrackerMgr::GetStorageStat(ConnectionInfo *pTrackerServer, TCHAR *pszGroupName, FDFSStorageStat *pStat, UINT32 nLen, UINT32 *pnStatCount) {
    UINT32 nRet;
    UINT64 nInBytes;
    ...
    size_t nNameLen = strlen(pszGroupName);
    ...
    if ((nRet = tcpsenddata_nb(pTrackerServer->sock, byOut, sizeof(TrackerHeader) + FDFS_GROUP_NAME_MAX_LEN, DEFAULT_NETWORK_TIMEOUT)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::GetStorageStat tcpsenddata_nb Header ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    if ((nRet = tcprecvdata_nb(pTrackerServer->sock, &resp, sizeof(resp), DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::GetStorageStat tcprecvdata_nb Header ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    *pnStatCount = static_cast<UINT32>(nInBytes / sizeof(StorageStat));
    ...
    if ((nRet = tcprecvdata_nb(pTrackerServer->sock, rawStat, nInBytes, DEFAULT_NETWORK_TIMEOUT, &nCount)) != 0) {
        ...
        LeaveCriticalSection(&pTrackerServer->csSend);
        WriteLogInfo(LogFileName, FDFSC_ERROR_MODE,
                     _T("TrackerMgr::GetStorageStat tcprecvdata_nb Body ")
                     _T("Failed, Error Code:%d"),
                     nRet);
        return enumNetworkError_FDFS;
    }
    ...
    for (UINT32 i = 0; i < *pnStatCount; i++) {
        ...
        strcpy_s(pDest->szDomainName, sizeof(pDest->szDomainName), (const char *)pSrc->byDomainName);
        strcpy_s(pDest->szVersion, sizeof(pDest->szVersion), (const char *)pSrc->byVersion);
